...
remove_rows_columns:
  ...
  remove_start:
    ...
    beq $t5, 13, remove_end         # if t5 reached all rows (39 pixels total, 13 rows of capsule halves) (goes from 0 to 12, then add 1 so 13)
    j remove_start
...
check_row:
  ...
  save_address:
    ...
    # push ($t2)              # push t2 onto stack for safekeeping            # might not even need
    j continue_loop_common
  ...
  row_loop_end:       # checked entire row
    ...
    # else, return to where check_row called
    j remove_row_end
...
# set t0 to 0 if nothing removed; set t0 to one if blocks removed
